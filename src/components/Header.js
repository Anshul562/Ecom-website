  ...
  console.log(getSession)
    ...
    useEffect(() => {
        ...
        $(function () {
            ...
            $(".btn-show-left-area").on(function () {
                $(".left-area").removeClass("show");
                $(".left-area").addClass("show");
            });


            $(".btn-close-left").on(function () {
                $(".left-area").removeClass("show");
            });
        })
    }, []);
    return (
        <>
                ...
                {getSession ? `Hello, ${getSession.user.name}`:'sign in'}
                    <svg onClick={!getSession? signIn:signOut} xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" className="feather feather-log-out" viewBox="0 0 24 24">
                        <defs />
